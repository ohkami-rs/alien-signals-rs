...
impl<T> SpyFn<T> {
    ...
    fn to_have_returned_with<V>(&self, value: V) -> bool where T: PartialEq<V> {
        self.last_returned_value.lock().unwrap().as_ref().is_some_and(|it| *it == value)
    }
}
