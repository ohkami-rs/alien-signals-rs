...
pub fn set_active_sub(sub: Option<Node>) -> Option<Node> {
    SYSTEM.with_borrow_mut(|sys| std::mem::replace(
        &mut sys.active_sub,
        sub
    ))
}
...
pub(crate) fn link(dep: Node, sub: Node, version: LinkVersion) {
    ...
    if let Some(next_dep) = next_dep && next_dep.dep() == dep {
    ...
    if let Some(prev_sub) = prev_sub && prev_sub.version() == version && prev_sub.sub() == sub {
        return;
    }
...
pub(crate) fn unlink(link: Link, sub: Node) -> Option<Link> {
    ...
    } else if { dep.set_subs(next_sub); next_sub.is_none() } {
        super::unwatched(dep);
    }
...
pub(crate) fn check_dirty(mut link: Link, mut sub: Node) -> bool {
    ...
    'top: loop {
        ...
        if (sub.flags() & Flags::DIRTY).is_nonzero() {
            ...
            if super::update(dep) {
                let subs = dep.subs().expect("BUG: no `dep.subs` in `MUTABLE | DIRTY` path");
            ...
            link = dep.deps().expect("BUG: no `dep.deps` in `MUTABLE | PENDING` path");
            sub = dep;
            check_depth += 1;
            continue;
        }
        ...
        while check_depth > 0 {
            ...
            if has_multiple_subs {
                link = stack.pop().expect("BUG: no `stack` item in `has_multiple_subs` path");
            } else {
                link = first_sub;
            }
    ...
    loop {
        ...
        if (flags & (Flags::PENDING | Flags::DIRTY)) == Flags::PENDING {
            ...
            if (flags & (Flags::WATCHING | Flags::RECURSED_CHECK)) == Flags::WATCHING {
                super::notify(sub.try_into().expect("BUG: `sub` is not effect in `shallow_propagate`"));
            }
        }
